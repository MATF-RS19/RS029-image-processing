...
public:
	kmeans(const img::Image<img::Type::GRAYSCALE>& img, int maxiter = 100)
		: m_img(img), m_output(m_img.rows(), m_img.cols()), m_maxiter(maxiter), m_f(0)
	{}
	...
	const img::Image<img::Type::GRAYSCALE>& cluster()
	{
		...
		std::cout << m_centroids.first << std::endl;
		std::cout << m_centroids.second << std::endl;
		std::cout << "----------------------" << std::endl;
...
img::Image<img::Type::GRAYSCALE> color_quantization(const img::Image<img::Type::GRAYSCALE>& img)
{
...
int main()
{
	...
	// for (int i = 0; i < 10; i++) {
		auto output = color_quantization(img);
	// }
