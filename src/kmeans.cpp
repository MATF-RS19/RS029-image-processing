...
class kmeans {
public:
	kmeans(const img::Image<img::Type::RGB>& img, int k)
...
private:
	...
	void update_help(unsigned long long& f, std::vector<vec3>& new_centroids, std::vector<int>& counter, int from, int to)
	{
		int k = m_centroids.size();
	...
	unsigned long long update()
	{
		...
		img::start_threads(0, m_img.rows(), &kmeans::update_help, this, f, new_centroids, counter);
...
int main()
{
	...
	// for (int i = 0; i < 10; i++) {
		auto output = color_quantization(img, 4);
	// }
